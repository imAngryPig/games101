...
public:
    MeshTriangle(const std::string& filename)
    {
        ...
        Vector3f max_vert = Vector3f{-std::numeric_limits<float>::infinity(),
                                     -std::numeric_limits<float>::infinity(),
                                     -std::numeric_limits<float>::infinity()};
        for (int i = 0; i < mesh.Vertices.size(); i += 3) {   //挨个三角形面遍历
            ...
            } //找寻每个三角形面的bounding box  用face_vertices来存三角形三个顶点信息
            ...
            new_mat->Kd = 0.6;
            new_mat->Ks = 0.0;
            new_mat->specularExponent = 0;  // 只有漫反射，无高光

            triangles.emplace_back(face_vertices[0], face_vertices[1],
                                   face_vertices[2], new_mat);  //存储每个三角形信息，并指定材质信息
        }

        bounding_box = Bounds3(min_vert, max_vert);  // 找到整个Mesh的bounding box

        std::vector<Object*> ptrs;
        for (auto& tri : triangles)
            ptrs.push_back(&tri);

        // 返回一个已经创建好的bvh的结构，里面的root就是bvh树的根
    ...
    Bounds3 getBounds() { return bounding_box; }  // 返回Mesh的整个boundingbox
...
inline Bounds3 Triangle::getBounds() { return Union(Bounds3(v0, v1), v2); } //返回这个三角形的boundingbox
