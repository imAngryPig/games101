...
void Scene::buildBVH(BVHAccel::SplitMethod kind) 
{
    printf(" - Generating BVH...\n\n");
    if(kind==BVHAccel::SplitMethod::NAIVE)
    this->bvh = new BVHAccel(objects, 1, BVHAccel::SplitMethod::NAIVE);
    else if(kind==BVHAccel::SplitMethod::SAH)
    this->bvh = new BVHAccel(objects, 1, BVHAccel::SplitMethod::SAH);
}

// important!!! 求ray与整个场景的交点
...
Vector3f Scene::castRay(const Ray &ray, int depth) const
{
    ...
    Intersection intersection = Scene::intersect(ray); // 获取相交点信息
    Material *m = intersection.m; // 获取材质
    Object *hitObject = intersection.obj;  // 获取打到的目标
    Vector3f hitColor = this->backgroundColor;   //  颜色初始化，如果光线没有打到任何物体，就上背景色
...
//        Vector3f tmp = hitPoint;
        switch (m->getType()) {
            ...
            default:
            {
                ...
                {
                    AreaLight* area_ptr = dynamic_cast<AreaLight*>(this->get_lights()[i].get());
