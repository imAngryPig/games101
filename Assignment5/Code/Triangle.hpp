...
public:
                              ...
                              Vector2f& st) const override
    {
        ...
        const Vector3f& v2 = vertices[vertexIndex[index * 3 + 2]]; // 获取对应的三角形网格的顶点
        Vector3f e0 = normalize(v1 - v0);
        Vector3f e1 = normalize(v2 - v1);
        N = normalize(crossProduct(e0, e1)); // 找法向量
        const Vector2f& st0 = stCoordinates[vertexIndex[index * 3]];
        const Vector2f& st1 = stCoordinates[vertexIndex[index * 3 + 1]];
        const Vector2f& st2 = stCoordinates[vertexIndex[index * 3 + 2]]; // 找对应三个点的纹理坐标
        st = st0 * (1 - uv.x - uv.y) + st1 * uv.x + st2 * uv.y;  // 插值算这个点的纹理坐标  此处三个系数和上面的算相交的实现要对应起来
    }

    Vector3f evalDiffuseColor(const Vector2f& st) const override
    {
        float scale = 5;
        bool pattern = (fmodf(st.x * scale, 1) > 0.5) ^ (fmodf(st.y * scale, 1) > 0.5);
